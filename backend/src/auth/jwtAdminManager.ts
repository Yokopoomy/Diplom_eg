...
export class JwtAdminManager extends AuthGuard('jwt') {
  private readonly logger = new Logger(JwtAdminManager.name);
  private readonly allowedRoles = ['admin', 'manager'];

  canActivate(context: ExecutionContext) {
    this.logger.log('Checking if user can activate (Admin/Manager)');
  ...
  handleRequest(err: any, user: any, info: any) {
    ...
    if (!this.allowedRoles.includes(user.role)) {
      this.logger.warn(`User with role ${user.role} tried to access admin/manager route`);
